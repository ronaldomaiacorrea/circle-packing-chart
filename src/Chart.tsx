  ...
  useLayoutEffect(() => {
    ...
    function createHierarchy(): d3.HierarchyNode<SimpleNode | HierarchyData> {
      return d3
        ...
        .sum((d) => {
          // Se é um nó folha (tem value), retorna o value
          if (d && typeof d === 'object' && 'value' in d && typeof d.value === 'number') {
            return d.value;
          }
          // Se é um nó pai (tem children) ou root, retorna 0 (será somado dos filhos)
          return 0;
        })
        .sort((a, b) => (b.value || 0) - (a.value || 0));
    }

    // Flatten hierarchy for ECharts dataset
    ...
    function overallLayout(params: echarts.CustomSeriesRenderItemParams, api: echarts.CustomSeriesRenderItemAPI) {
      ...
      // Criar uma nova hierarquia e aplicar o pack
      const hierarchy = createHierarchy();

      // O d3.pack() transforma o HierarchyNode em HierarchyCircularNode
      displayRoot = d3
    ...
    function drillDown(targetNodeId?: string) {
      ...
      if (targetNodeId != null) {
        ...
        if (found) {
          // Criar uma nova hierarquia com o nó encontrado como raiz
      ...
      // Aplicar pack na hierarquia escolhida
      displayRoot = d3
        .pack<SimpleNode | HierarchyData>()
    ...
    // Initial setup - inicializar displayRoot aqui
    displayRoot = d3
      .pack<SimpleNode | HierarchyData>()
      .size([width - 2, height - 2])
